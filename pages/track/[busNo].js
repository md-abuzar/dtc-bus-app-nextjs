...
const RouteDetails = ({ busNo, routeDetails, liveLocation }) => {
        ...
        <div className="row">
          ...
          <div className="col-sm-7 ">
            <div className="map-container">
              <Map stops={routeDetails.stops} liveLocation={liveLocation} />
            </div>
          </div>
        </div>
      </div>
...
export async function getServerSideProps({ params }) {
  ...
  try {
    ...
    if (liveLocationResponse.ok) {
      liveLocation = await liveLocationResponse.json();
    } else {
      console.error('Failed to fetch live location:', liveLocationResponse.status);
    }
